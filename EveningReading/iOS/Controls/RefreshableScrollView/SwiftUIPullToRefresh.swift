...
//  EveningReading (iOS)
//
//  Created by Chris Hodge on 5/17/22.
//
...
public struct PullToRefreshView<Progress, Content>: View where Progress: View, Content: View {
...
public extension PullToRefreshView where Progress == RefreshActivityIndicator {
...
public struct RefreshableCompat<Progress>: ViewModifier where Progress: View {
    ...
    public func body(content: Content) -> some View {
        PullToRefreshView(showsIndicators: showsIndicators,
                              loadingViewBackgroundColor: loadingViewBackgroundColor,
                              threshold: threshold,
                              onRefresh: onRefresh,
                              progress: progress) {
...
struct TestViewWithCustomProgress: View {
    ...
    var body: some View {
        PullToRefreshView(onRefresh: { done in
          DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
            self.now = Date()
            done()
          }
