                    ...
                    VStack (alignment: .leading) {
                        VStack (alignment: .leading) {
                            Text("Shack Community Guidelines")
                                .font(.title2)
                                .bold()
                                .foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            Spacer().fixedSize()
                        ...
                        VStack (alignment: .leading) {
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("No threats of violence.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("No personal attacks or insults.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("No obnoxious ascii art.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("No overused memes (if you question if it’s overused, it is).").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("No spam.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("No sharing or linking to pirate sites, leaked or stolen media, and warez. Discussion of piracy as a topic is fine.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("No gross, disgusting media including gore.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("Topics that have been done to death at Shacknews ie. tipping, circumcision, etc. Let’s just all agree to disagree.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("Following the same rules we use for racism and homophobia, we'll be nuking posts we feel to be overtly sexist or extremely misogynistic.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                            HStack (alignment: .top) {
                                Text("• ").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                                Text("Posts consisting solely of link dumps to porn are subject to the 'contextless post' rule and will be removed.").font(.body).fontWeight(.light).foregroundColor(appSession.isDarkMode ? Color.white : Color.primary)
                            }
                        }
                        VStack (alignment: .leading) {
                            Spacer().fixedSize()
