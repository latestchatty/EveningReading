                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack {
                        ForEach(filteredThreads(), id: \.threadId) { thread in
                            NavigationLink(destination: ThreadDetailView(threadId: .constant(thread.threadId), postId: .constant(0), replyCount: .constant(thread.posts.count - 1)), tag: thread.threadId, selection: $selectedThreadId) { EmptyView() }
